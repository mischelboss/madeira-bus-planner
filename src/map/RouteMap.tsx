...
/** Standalone map for one route: its stop-string polyline + markers + live "you". */
export function RouteMap({ stops, color }: { stops: BrowseStop[]; color: string }) {
  ...
  useEffect(() => {
    ...
    const points: Feature[] = stops.map((s, i) => ({
      ...
      properties: { kind: i === 0 ? "origin" : i === stops.length - 1 ? "dest" : "mid", name: s.name },
    }));
